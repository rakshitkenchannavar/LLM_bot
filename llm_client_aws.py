...
# ------------------------------------------------------------------
# Provider: Google Gemini (DISABLED on this machine - key commented out)
# To re-enable later: uncomment the 4 lines marked [GEMINI KEY]
# ------------------------------------------------------------------

class GeminiProvider:
    """Google Gemini provider.

    NOTE: On this machine the Gemini API key is commented out and the
    provider is inert. Use LLM_PROVIDER=bedrock in .env.
    """
    ...
    def __init__(self):
        # [GEMINI KEY] self.api_key = Config.GEMINI_API_KEY
        # [GEMINI KEY] if not self.api_key:
        # [GEMINI KEY]     raise ValueError("GEMINI_API_KEY is not set in .env")
        self.api_key = ""  # Gemini disabled on this machine; use LLM_PROVIDER=bedrock
        ...
        logger.info("Gemini provider present but DISABLED (key commented out)")
    ...
    def _discover_models(self):
        ...
        try:
            resp = requests.get(
                self.BASE_URL,
                # [GEMINI KEY] params={"key": self.api_key},
                params={},
                timeout=30,
            )
    ...
    def invoke(self, prompt, system_prompt=None, max_tokens=None, temperature=None):
        ...
        for model in candidates:
            # [GEMINI KEY] url = f"{self.BASE_URL}/{model}:generateContent?key={self.api_key}"
